"""model_builder_sandra.py - Phase 4 modification.

Trains and saves a penguin species classifier (modified version).
...
Process:
    ...
    - Split into train and test sets (modified test size).
    - Train a RandomForestClassifier.
    - Evaluate on held-out test data.
    - Save the trained model to artifacts/model_sandra.joblib.

Terminal command to run this file:

    uv run python -m mlstudio.model_builder_sandra

Modification:
    - Increased test size from 0.20 to 0.25
    - Saved model to a new file (model_sandra.joblib)
"""
...
MODEL_PATH: Final[Path] = Path("artifacts") / "model_sandra.joblib"
